...
public class Reservation {
    ...
    public void updateDates(Date checkIn, Date checkOut) throws DomainException { // Define que o método pode lançar uma exceção
        Date now = new Date();

        if (!(checkIn.after(now) || checkOut.after(now))){
            throw new DomainException("Error in reservation: Reservation dates for " +
                    "update must be future dates"); // Quem deve tratar a exceção é no catch da Main
        }
